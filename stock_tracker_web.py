...
SCHWAB_TOKEN    = os.environ.get("SCHWAB_TOKEN_JSON", "")  # full token JSON as env var
SCHWAB_CALLBACK = "https://127.0.0.1:8182"
TOKEN_FILE      = os.path.join(os.path.dirname(os.path.abspath(__file__)), "schwab_token.json")
...
def get_schwab_client():
    ...
    try:
        ...
        # Option 1: Token JSON stored as environment variable (for Railway)
        if SCHWAB_TOKEN and SCHWAB_APP_KEY and SCHWAB_SECRET:
            try:
                token_data = json.loads(SCHWAB_TOKEN)
                with open(TOKEN_FILE, "w") as f:
                    json.dump(token_data, f)
            except Exception as e:
                print(f"Could not write token from env: {e}")

        # Option 2: Token file on disk
        if os.path.exists(TOKEN_FILE) and SCHWAB_APP_KEY and SCHWAB_SECRET:
            try:
                _schwab_client = schwab.auth.client_from_token_file(
                    TOKEN_FILE, SCHWAB_APP_KEY, SCHWAB_SECRET)
                return _schwab_client
            except Exception as e:
                print(f"Token file load failed: {e}")

        return None  # No Schwab auth available, fall back to yFinance
    except ImportError:
        return None


def fetch_ticker_schwab(client, t):
    """Fetch ticker data using Schwab API."""
    ...
    if resp.status_code != 200:
        raise ValueError(f"Schwab history failed for {t}: {resp.status_code}")
    data = resp.json()
    candles = data.get("candles", [])
    if not candles:
        raise ValueError(f"No candles for {t}")
    df = pd.DataFrame(candles)
    df["datetime"] = pd.to_datetime(df["datetime"], unit="ms")
    df = df.set_index("datetime")
    df = df.rename(columns={"close":"Close","high":"High","open":"Open","low":"Low"})

    # Real-time quote
    try:
        qresp = client.get_quote(t)
        qdata = qresp.json().get(t, {})
        price = (qdata.get("quote", {}).get("lastPrice") or
                 qdata.get("quote", {}).get("mark") or
                 float(df["Close"].iloc[-1]))
        week52_high = float(qdata.get("quote", {}).get("52WeekHigh", 0) or 0)
    except Exception:
        price = float(df["Close"].iloc[-1])
        week52_high = 0

    return df, float(price), week52_high
...
def fetch_ticker(t):
    ...
    if schwab:
        # ── Schwab path (real-time) ──
        try:
            df, price, week52_high = fetch_ticker_schwab(schwab, t)
            hist_ath = float(df["High"].max()) if "High" in df.columns else float(df["Close"].max())
            cached = _ath_cache.get(t, 0)
            ath = max(hist_ath, week52_high, cached)
            _ath_cache[t] = ath
            ...
            print(f"Schwab fetch failed for {t}, falling back to yFinance: {e}")
            schwab = None
...
def fetch_all_sync(tickers, loop):
    ...
    # Update data badge based on source
    sources = set(d.get("source","yfinance") for d in results.values())
    source_label = "REAL-TIME (Schwab)" if "schwab" in sources else "DELAYED (yFinance)"
    asyncio.run_coroutine_threadsafe(
        broadcast({"type": "source_update", "label": source_label}), loop)
